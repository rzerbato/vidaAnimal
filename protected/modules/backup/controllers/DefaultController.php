...
class DefaultController extends Controller
{
	...
	public function StartBackup($addcheck = true)
	{
		...
		$this->file_name =  $this->path . $this->back_temp_file . date('d.m.Y_H.i.s') . '.sql';
	...
	public function actionCreate()
	{
		...
		/**
		 * Copio las imagenes a la carpeta de backup
		 */
		$from = 'C:\\xampp\htdocs\veterinaria\images\*.*';
		//Creo el directorio de destino
		date_default_timezone_set('America/Argentina/Cordoba');
		mkdir('C:\\backup\imagenes_'.date('d.m.Y_H.i.s'), 0777, true);
		$to = 'C:\\backup\imagenes_'.date('d.m.Y_H.i.s');
		...
		switch( $this->action->id)
		{
			case 'restore':
				{					
					...
					$this->menu[] = array('label'=>Yii::t('app', '<i class="fa fa-briefcase fa-fw"></i> Administrar Backups'), 'url'=>array('index'));
				}
			case 'create':
				{
					//$this->menu[] = array('label'=>Yii::t('app', 'List Backup') . ' ' . $model->label(2), 'url'=>array('index'));
			...
			default:
				{
					//$this->menu[] = array('label'=>Yii::t('app', 'List Backup') . ' ' . $model->label(2), 'url'=>array('index'));
					$this->menu[] = array('label'=>Yii::t('app', '<i class="fa fa-plus fa-fw"></i> Crear Backup') . ' ' . $model->label(), 'url'=>array('create'));
					//$this->menu[] = array('label'=>Yii::t('app', '<i class="fa fa-cloud-upload"></i> Subir Backup') . ' ' . $model->label(), 'url'=>array('upload'));
