...
'actions'=>array('index','view', 'buscarMarcas', 'listadodinamico'),
'users'=>array('*'),
),
...
public function actionBuscarMarcas(){
  ...
  $data = new CActiveDataProvider(Marca::model(), array(
    'criteria'=>$criteria,
    'pagination'=>array(
      'pageSize'=>100,
    ),
...
public function actionListadodinamico()
{
 if (isset($_POST['Producto']['rubro_idRubro']))
 {
  ...
  echo CHtml::tag('option', array('value'=>0), 'Seleccionar Marca', true);
  foreach($data->getData() as $value){
    //print_r($value);
    
    echo CHtml::tag('option', array('value'=>$value['idMarca']), CHtml::encode($value['nombre']), true);
  }
